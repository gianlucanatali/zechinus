...
entry works the same way — see "Package boundary" below.)
...
optimistic update and throws `OptimisticLockConflictError` (from `zechinus/react`) —
...
- **The explicit goal:** a Swift (or any other language) implementation for the mobile
  ...
  spec — never needing to read `zechinus/core/aggregation.ts` itself.
...
- **Anti-overlap guard**: a new rotation can't start before the previous one's old epoch is
  ...
  `zechinus/adapters/dekRotationCoordinator.ts`.
...
takes a dependency on it exactly like any other npm package (see "Installation" above)
and imports it as a bare specifier: `import { defineStore } from "zechinus"`. There is
**no bundler-specific path alias** — resolution goes entirely through the package's own
`exports` map. A consumer that happens to be a monorepo can instead pull this package in
as an npm workspace member (`node_modules/zechinus` becomes a symlink instead of a real
copy) — nothing about how you import from it changes either way.
...
```ts
import { supabaseStorageAdapter } from "zechinus/adapters/supabaseStorageAdapter.ts";
import { pgStorageAdapter } from "zechinus/adapters/pgStorageAdapter.ts";
import { webauthnKeyProvider } from "zechinus/adapters/webauthnKeyProvider.ts";
import { mnemonicRecovery } from "zechinus/adapters/mnemonicRecovery.ts";
import { createWorkerKeyHandle } from "zechinus/adapters/workerKeyHandle.ts";
...
1. Update `zechinus/examples/basic-usage.ts` FIRST until it compiles again and the test
   passes.
2. Mirror the change into THIS README (the "Cardinality"/"What Zechinus doesn't do yet"
...
3. If the change is a new capability (new cardinality, new encryption mode), add a line to
   the "What Zechinus doesn't do yet" table → move it to "Cardinality"/Quickstart once
