...
export function supabaseStorageAdapter(
  ...
  return {
    ...
    /**
     * `expectedHash: null` → plain INSERT; a unique-constraint violation (Postgres
     * SQLSTATE 23505, surfaced by PostgREST as `error.code`) is a conflict → `false`,
     * never thrown. `expectedHash` set → `UPDATE ... WHERE ... AND content_hash =
     * expected`, `.select()` to see which rows matched — zero rows back means either
