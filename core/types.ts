/**
 * Types/ports of the secure-store framework (Zechinus).
 *
...
/**
 * The minimal crypto contract Zechinus needs — NOT a specific app's full key-handle
 ...
 * works with Zechinus (TypeScript's structural typing makes this automatic — no
 ...
 * style choice: Zechinus only ever encrypts data E2E under a per-user DEK, so by
...
export interface StorageAdapter {
   ...
   * check (`zechinus/core/aggregation.ts`) to verify several never-observed-this-
 ...
 * Zechinus owns key construction (`<storeName>:<userId>`), the adapter maps it to
